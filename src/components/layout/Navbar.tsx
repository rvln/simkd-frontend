...
export const Navbar = () => {
    ...
    const fetchTicker = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/user/ticker-stats`,
          {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed left-0 right-0 z-50 bg-surface/80 backdrop-blur-md border-b border-outline-variant/20 shadow-sm transition-all duration-300 ${tickerMessages.length > 0 ? "top-8" : "top-0"}`}
      >
